...
/**
 * @author ckaratza Tries to interrogate vertx router and build an OpenAPI specification. Tries to
 *     interrogate handlers of route with OpenApi Operation methods and cross-reference with route
 *     information. By no means all OpenApi 3 spec is covered, so this part will be adjusted based
 *     on use cases encountered.
 */
public final class OpenApiSpecGenerator {
  ...
  public static OpenAPI generateOpenApiSpecFromRouter(
      Router router, String title, String version, String serverUrl) {
        ...
        .forEach(
            route -> {
              ...
              if (pathItem != null) {
                ...
                operations.forEach(
                    operation -> operation.setParameters(extractPathParams(route.getPath())));
              }
            });
    processHandlerAnnotations(router, paths);
  }

  private static void processHandlerAnnotations(Router router, Map<String, PathItem> paths) {
            ...
            route -> {
              try {
                ...
                List<Handler<RoutingContext>> handlers =
                    (List<Handler<RoutingContext>>) contextHandlers.get(route);
                handlers.forEach(
                    handler -> {
                      try {
                        Class<?> delegate = handler.getClass().getDeclaredField("arg$1").getType();
                        Arrays.stream(delegate.getDeclaredMethods())
                            .distinct()
                            .forEach(
                                method -> {
                                  ...
                                  if (null != annotation) {
                                    ...
                                    if (matchedOperation != null
                                        && (annotation
                                            .operationId()
                                            .equals(route.getPath().substring(1)))) {
                                      AnnotationMappers.decorateOperationFromAnnotation(
                                          annotation, matchedOperation);
                                      RequestBody body =
                                          method.getParameters()[0].getAnnotation(
                                              RequestBody.class);
                                      if (body != null) {
                                        matchedOperation.setRequestBody(
                                            AnnotationMappers.fromRequestBody(body));
                                      }
                                    }
                                  }
                                });
